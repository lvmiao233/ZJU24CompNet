...
sidebar_position: 1
title: Wireshark安装
description: 一份详尽的Wireshark安装指南，为Windows、Linux和macOS用户提供清晰的下载与安装指导。文档引导用户访问Wireshark官网，根据不同系统架构选择合适的安装包，并对安装过程中的关键步骤和注意事项进行了详细说明，旨在帮助用户顺利完成安装。
---
...
<LinkCard title="Wireshark官方网站" url="https://www.wireshark.org/">The world's most popular network protocol analyzer</LinkCard>
<br/>

    ![Wireshark 官网下载页面截图（Download 按钮）](img/image-20240905022017410.png)
...
* 页面将跳转至下载选项，请根据系统情况选择对应版本
  ...
  * ARM架构：Windows Arm 64 Installer
    ...
    ![Wireshark 下载选项页面截图（选择安装包）](img/image-20240905022127355.png)
...
* 打开下载完成的WireShark安装包，阅读并同意EULA
  ...
  ![Wireshark 安装向导 EULA 协议页面截图（Windows）](img/image-20240905022644185.png)

* 选择要安装的组件，请至少选择默认组件

  ![Wireshark 安装组件选择界面截图（Windows）](img/image-20240905022618017.png)

* 配置附加任务，建议选择扩展名关联，以便快速打开捕捉记录文件

  ![Wireshark 安装附加任务配置界面截图（扩展名关联，Windows）](img/image-20240905022750428.png)

* 选择合适的安装目录

  ![Wireshark 安装目录选择界面截图（Windows）](img/image-20240905022915255.png)

* 选择安装Npcap，该项为网络抓包用途必选

  ![Npcap 安装选项界面截图（Wireshark 安装向导，Windows）](img/image-20240905022950288.png)

* 如无捕捉USB通讯流量需求，无需选择安装USBcap

  ![USBcap 安装选项界面截图（可选，Windows）](img/image-20240905023107622.png)

* 安装过程中，安装向导将同时启动Npcap安装，可保持默认选项

  ![Npcap 安装向导界面截图（保持默认选项，Windows）](img/image-20240905023214089.png)

* Npcap安装完成后，请留意完成安装向导，否则将持续阻塞Wireshark安装过程

  ![Npcap 安装完成提示界面截图（请完成向导，Windows）](img/image-20240905023414829.png)
...
![开始菜单中的 Wireshark 图标截图（带 Tux 角标，Linux）](img/image-20240905025149526.png)
...
* 打开下载完成的WireShark安装包，将Wireshark拖动到Applications中进行安装
  ...
  ![macOS 安装：拖动 Wireshark 到 Applications 的界面截图](img/image-20240905030522318.png)

* 安装ChmodBPF

  ![macOS 安装 ChmodBPF 工具界面截图](img/image-20240905030602591.png)
