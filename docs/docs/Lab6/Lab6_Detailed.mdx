...
import DeadlineProgress from '@site/src/components/DeadlineProgress';
import TaskCard from '@site/src/components/TaskCard';
import StepRecordCard from '@site/src/components/StepRecordCard';
import AnswerInput from '@site/src/components/AnswerInput';
import ExportButton from '@site/src/components/ExportButton';
import GridContainer from '@site/src/components/GridContainer';

<h3 style={{color: '#006d75', marginTop: 0, marginBottom: 8}}>实验资源</h3>
<DeadlineProgress start={'2025-12-09 18:50:00'} end={'2025-12-23 23:59:59'}/>
...
<GridContainer>
    ...
    <FileCard file_type={'pdf'} name={'Lab6 动态路由协议BGP配置 实验课件（原理）'} size={'2263394'} link={require('@site/assets/slides/Lab6 动态路由BGP协议.pdf').default} />
    <FileCard file_type={'pdf'} name={'Lab6 动态路由协议BGP配置 实验课件（命令）'} size={'1721069'} link={require('@site/assets/slides/Lab6_BGP路由协议.pdf').default} />
</GridContainer>
...
<iframe className="cf-radar-embed" width="800" height="438" src="https://radar.cloudflare.com/embed/BgpRoutingStats?locale=zh-CN&widgetState=%7B%7D&ref=%2Fzh-cn%2Frouting" title="Cloudflare Radar - 路由统计信息" loading="lazy" style={{border: 0, width: '100%'}}></iframe>
...
1.  在R3上使用命令查看BGP邻居关系（命令：`show ip bgp neighbor`），观察得知，邻居的IP是<AnswerInput size="large" questionId="Lab6-p1-s5-r3-ip"/>，链路类型属于<AnswerInput size="medium" questionId="Lab6-p1-s5-r3-type"/>，状态是<AnswerInput size="medium" questionId="Lab6-p1-s5-r3-state"/>，但现象是没有活动的TCP连接
    <StepRecordCard 
...
2.  在R5上使用命令查看BGP邻居关系（命令：`show ip bgp neighbor`），观察得知，邻居的IP是<AnswerInput size="large" questionId="Lab6-p1-s5-r5-ip"/>，链路类型属于<AnswerInput size="medium" questionId="Lab6-p1-s5-r5-type"/>，状态是<AnswerInput size="medium" questionId="Lab6-p1-s5-r5-state"/>，但现象是没有活动的TCP连接
    <StepRecordCard 
      questionId="Lab6-p1-s5" 
      title="R5邻居关系示例" 
      uploadOptions={[
    ...
    />
3.  观察发现状态始终无法到达 `Established`，此时开启调试开关（命令：`debug ip bgp`），查看控制台输出的错误信息，可见错误原因是被对方拒绝连接，是因为R3默认使用了**物理出接口**的IP地址作为源地址，而R5配置的邻居地址是R3的<AnswerInput size="medium" questionId="Lab6-p1-s5-interface"/>，因邻居地址不符被拒绝
    <StepRecordCard 
      questionId="Lab6-p1-s5" 
      title="调试信息示例" 
      uploadOptions={[
...
2.  等待几十秒，在R3上使用 `show ip bgp neighbor` 查看邻居关系，确认状态是否变为 `ESTAB`（已建立）；观察得知，与R5的邻居关系已经建立，对方的连接端口是<AnswerInput size="medium" questionId="Lab6-p1-s6-r3-port"/>
    <StepRecordCard 
      questionId="Lab6-p1-s6" 
      title="R3邻居关系示例" 
      uploadOptions={[
    ...
    >
      ![R3邻居关系建立后截图1](img/image9.png)
      ![R3邻居关系建立后截图2](img/image10.png)
    </StepRecordCard>
3.  在R5上使用 `show ip bgp neighbor` 查看邻居关系，确认状态是否变为 `Established`（已建立）；观察得知，与R3的邻居关系已经建立，对方的连接端口是<AnswerInput size="medium" questionId="Lab6-p1-s6-r5-port"/>
    <StepRecordCard 
      questionId="Lab6-p1-s6" 
      title="R5邻居关系示例" 
      uploadOptions={[
...
1.  在R3上使用 `show ip bgp` 命令查看BGP数据库，标注iBGP路由；观察可知，存在<AnswerInput size="small" questionId="Lab6-p1-s7-r3-count"/>条状态码=r的路由（表示没有成功写入路由表）
    <StepRecordCard 
...
2.  在R5上使用 `show ip bgp` 命令查看BGP数据库，标出iBGP路由
    ...
    />
3.  在R3上使用 `show ip route` 查看路由表；观察可知，网络地址<AnswerInput size="large" questionId="Lab6-p1-s7-r3-net1"/>、<AnswerInput size="large" questionId="Lab6-p1-s7-r3-net2"/>在路由表中已存在比BGP优先级高的OSPF路由，所以BGP的路由信息没有成功写入
    <StepRecordCard 
      questionId="Lab6-p1-s7" 
      title="R3路由表示例" 
      uploadOptions={[
...
1. 在R1上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R1的两个邻居的IP分别为<AnswerInput size="large" questionId="Lab6-p2-s10-r1-ip1"/>、<AnswerInput size="large" questionId="Lab6-p2-s10-r1-ip2"/>，链路类型均为<AnswerInput size="medium" questionId="Lab6-p2-s10-r1-type"/>
    <StepRecordCard 
...
2. 在R2上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R2邻居的IP分别为<AnswerInput size="large" questionId="Lab6-p2-s10-r2-ip1"/>、<AnswerInput size="large" questionId="Lab6-p2-s10-r2-ip2"/>，链路类型均为<AnswerInput size="medium" questionId="Lab6-p2-s10-r2-type"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R2邻居关系示例" 
      uploadOptions={[
    ...
    />
3. 在R3上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R3的iBGP邻居的IP为<AnswerInput size="large" questionId="Lab6-p2-s10-r3-igp"/>，eBGP邻居的IP为<AnswerInput size="large" questionId="Lab6-p2-s10-r3-ebgp"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R3邻居关系示例" 
      uploadOptions={[
...
4. 在R5上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R5的iBGP邻居的IP为<AnswerInput size="large" questionId="Lab6-p2-s10-r5-igp"/>，eBGP邻居的IP为<AnswerInput size="large" questionId="Lab6-p2-s10-r5-ebgp"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R5邻居关系示例" 
      uploadOptions={[
    ...
    />
5. 在R6上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R6的两个邻居的IP分别为<AnswerInput size="large" questionId="Lab6-p2-s10-r6-ip1"/>、<AnswerInput size="large" questionId="Lab6-p2-s10-r6-ip2"/>，链路类型均为<AnswerInput size="medium" questionId="Lab6-p2-s10-r6-type"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R6邻居关系示例" 
      uploadOptions={[
    ...
    />
6. 在R7上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R7的两个邻居的IP分别为<AnswerInput size="large" questionId="Lab6-p2-s10-r7-ip1"/>、<AnswerInput size="large" questionId="Lab6-p2-s10-r7-ip2"/>，链路类型均为<AnswerInput size="medium" questionId="Lab6-p2-s10-r7-type"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R7邻居关系示例" 
      uploadOptions={[
    ...
    />
7. 在R8上查看邻居关系（命令：`show ip bgp neighbor`），标出Link类型和对方的IP、连接状态（找出关键信息进行截图）；观察可知，R8的两个邻居的IP分别为<AnswerInput size="large" questionId="Lab6-p2-s10-r8-ip1"/>、<AnswerInput size="large" questionId="Lab6-p2-s10-r8-ip2"/>，链路类型均为<AnswerInput size="medium" questionId="Lab6-p2-s10-r8-type"/>
    <StepRecordCard 
      questionId="Lab6-p2-s10" 
      title="R8邻居关系示例" 
      uploadOptions={[
...
2.  在路由器R1查看BGP数据库（命令：`show ip bgp`），注意观察`Path`一列，标出到达R2-R5间子网、R6-R7间子网、R7-R8间子网以及R2-R8间子网的最佳路由（标记为 > 的为最佳路由），并分析其经过的AS路径是否是最短的；观察可知：
    * 到达R2-R5间子网的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s11-r25-nh"/>，经过的AS路径为<AnswerInput size="medium" questionId="Lab6-p2-s11-r25-path"/>
    * 到达R6-R7间子网的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s11-r67-nh"/>，经过的AS路径为<AnswerInput size="medium" questionId="Lab6-p2-s11-r67-path"/>
    * 到达R7-R8间子网的路由有<AnswerInput size="small" questionId="Lab6-p2-s11-r78-count"/>条，其中最佳路由的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s11-r78-nh"/>，经过的AS路径最短，AS号依次为<AnswerInput size="medium" questionId="Lab6-p2-s11-r78-path"/>
    * 到达R8-R2间子网的路由有<AnswerInput size="small" questionId="Lab6-p2-s11-r82-count"/>条，其中最佳路由的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s11-r82-nh"/>，经过的AS路径最短，AS号依次为<AnswerInput size="medium" questionId="Lab6-p2-s11-r82-path"/>
...
* 到达R1-R3间子网的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s12-r13-nh"/>，经过的AS路径为<AnswerInput size="medium" questionId="Lab6-p2-s12-r13-path"/>
* 到达R7-R8间子网的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s12-r78-nh"/>，经过的AS路径为<AnswerInput size="medium" questionId="Lab6-p2-s12-r78-path"/>
* 到达R1-R6间子网的路由有<AnswerInput size="small" questionId="Lab6-p2-s12-r16-count"/>条，其中最佳路由的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s12-r16-nh"/>，经过的AS路径最短，AS号依次为<AnswerInput size="medium" questionId="Lab6-p2-s12-r16-path"/>
* 到达R6-R7间子网的路由有<AnswerInput size="small" questionId="Lab6-p2-s12-r67-count"/>条，其中最佳路由的下一跳是<AnswerInput size="medium" questionId="Lab6-p2-s12-r67-nh"/>，经过的AS路径最短，AS号依次为<AnswerInput size="medium" questionId="Lab6-p2-s12-r67-path"/>
...
1.  在路由器R6查看BGP数据库，记录当前到达R2-R5间子网的最佳路由、经过的AS路径（应走最短路）；观察可知，到达R2-R5间子网的最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p2-s15-before"/>
    <StepRecordCard 
...
3.  等待BGP收敛（可能需要几秒到一分钟），在R6上再次查看BGP数据库，观察到达R2-R5间子网的最佳路由是否切换到了备用路径；观察可知，到达R2-R5间子网的最佳路由的下一跳变为<AnswerInput size="large" questionId="Lab6-p2-s15-after"/>
    <StepRecordCard 
      questionId="Lab6-p2-s15" 
      title="R6的BGP数据库变化" 
      uploadOptions={[
...
2.  跟踪R1到该子网的路由（命令：`traceroute [ip-addr]`，如果需要提前终止可按Ctrl+6），观察数据包是在哪里丢弃的；观察可知路由追踪在路由器<AnswerInput size="medium" questionId="Lab6-p3-s16-router"/>中断了
    <StepRecordCard 
      questionId="Lab6-p3-s16" 
      title="路由跟踪示例" 
      uploadOptions={[
...
1.  查看R3的BGP数据库，标记到达R2-R8间子网的BGP最佳路由，确认R3知道如何转发；观察可知，到达R2-R8间子网的最佳路由的下一跳IP地址是<AnswerInput size="large" questionId="Lab6-p3-s17-r3-nh"/>
    <StepRecordCard 
...
2.  查看R3的路由表，观察可知，到达R2-R8间子网的下一跳IP地址<AnswerInput size="large" questionId="Lab6-p3-s17-r3-bgp"/>（属于R2）是由BGP写入的；去往该地址的下一跳IP地址<AnswerInput size="large" questionId="Lab6-p3-s17-r3-ospf"/>（属于R4）是由OSPF写入的
    <StepRecordCard 
      questionId="Lab6-p3-s17" 
      title="R3路由表示例" 
      uploadOptions={[
...
<details>
<summary>背景知识：BGP同步（避免路由黑洞）与重分发（消除路由黑洞）</summary>
...
然而同步规则只能避免路由黑洞，而不能消除路由黑洞，当我们必须经过该AS内部时，我们就需要通过重分发消除路由黑洞，即将BGP的路由注入内部的IGP(如OSPF)；不幸的是这在今天的互联网规模下已不再适用——启用同步意味着必须将全球数十万条BGP路由全部注入到内部的OSPF协议中（重分发），这会瞬间撑爆IGP路由器的内存和CPU，导致网络瘫痪；因此，现代Cisco IOS默认都是关闭同步且不启用重分发的；在现代网络架构中，我们通常采用**MPLS（多协议标签交换）**或**GRE隧道**技术，在R3和R5之间建立一条“隧道”直接穿过R4，使得R4只需要转发隧道封装包（基于AS内部标签或IP）而无需知道外部路由，从而优雅地解决黑洞问题，我们这里的重分发和开启同步更多处于教学演示原理的需要
...
2. 查看R3的BGP数据库；观察可知，到达R2-R8间子网的路由有<AnswerInput size="small" questionId="Lab6-p3-s18-r3-count"/>条；其中最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s18-r3-nh"/>（属于R1），因为同步功能打开后，BGP判断AS内部缺少相应的路由，因此不选择本AS作为转发路径
    <StepRecordCard 
      questionId="Lab6-p3-s18" 
      title="R3 BGP数据库示例" 
      uploadOptions={[
...
3.  查看R3的路由表；观察可知，到达R2-R8间子网的下一跳IP为<AnswerInput size="large" questionId="Lab6-p3-s18-r3-rt-ip"/>，属于路由器<AnswerInput size="medium" questionId="Lab6-p3-s18-r3-rt-router"/>
    <StepRecordCard 
      questionId="Lab6-p3-s18" 
      title="R3路由表示例" 
      uploadOptions={[
...
4.  查看R1的BGP数据库；观察可知，到达R2-R8间子网的最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s18-r1-nh"/>，属于路由器<AnswerInput size="medium" questionId="Lab6-p3-s18-r1-router"/>；由于使用了水平分裂方式，R3并没有向R1报告关于这个子网的路由，因为R3选的下一跳是R1
    <StepRecordCard 
      questionId="Lab6-p3-s18" 
      title="R1 BGP数据库示例" 
      uploadOptions={[
...
6.  跟踪R1到达R2-R8互联端口的路由；观察可知依次经过了路由器<AnswerInput size="medium" questionId="Lab6-p3-s18-tr1"/>、<AnswerInput size="medium" questionId="Lab6-p3-s18-tr2"/>、<AnswerInput size="medium" questionId="Lab6-p3-s18-tr3"/>、<AnswerInput size="medium" questionId="Lab6-p3-s18-tr4"/>
    <StepRecordCard 
      questionId="Lab6-p3-s18" 
      title="路由跟踪示例" 
      uploadOptions={[
...
解决路由黑洞更常见的方法是路由重分发（Redistribution），即将BGP学到的路由注入到IGP（OSPF）中，让内部路由器（R4）知道如何到达外部网络，从而也满足了同步的条件

:::tip 提示

如果你在R3/R5上宣告的网络范围覆盖了192.168.13.0/24、192.168.25.0/24，则这两个连接其他AS的网络也会被OSPF视为内部链路宣告，此时尽管BGP学习到了这两个网络的路由信息，也会因为RIB failure而不会成功重分发相关条目

因此第2/3步查看OSPF数据库时，相比示例图片缺少这两个网络的条目是正常现象，通常不影响后续实验步骤
...
2.  等一会儿，查看R3的OSPF数据库，确认出现了外部LSA；观察可知，OSPF从BGP中重分发了AS外部链路的信息，但是R3-R1的直连网络<AnswerInput size="large" questionId="Lab6-p3-s19-r3-net"/>没有被本路由器重分发
    <StepRecordCard 
      questionId="Lab6-p3-s19" 
      title="R3 OSPF数据库示例" 
      uploadOptions={[
...
3.  查看R5的OSPF数据库，确认出现了外部LSA；观察可知，OSPF从BGP中重分发了AS外部链路的信息，但是R5-R2的直连网络<AnswerInput size="large" questionId="Lab6-p3-s19-r5-net"/>没有被本路由器重分发
    <StepRecordCard 
      questionId="Lab6-p3-s19" 
      title="R5 OSPF数据库示例" 
      uploadOptions={[
...
4.  查看R4的路由表是否出现了AS外部的路由信息（O E2 类型）；观察可知，R4上增加了AS外部的路由信息，此时到达R2-R8间子网的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s19-r4-nh1"/>和<AnswerInput size="large" questionId="Lab6-p3-s19-r4-nh2"/>（优先级相同），因为重分发后OSPF将在AS内部传播BGP的外部路由信息
    <StepRecordCard 
      questionId="Lab6-p3-s19" 
      title="R4路由表示例" 
      uploadOptions={[
...
2.  等待一段时间后，在R1上查看BGP数据库观察到达R2-R8间子网的最佳BGP路由（应恢复为指向R3）；观察可知，到达R2-R8间子网的路由有<AnswerInput size="small" questionId="Lab6-p3-s20-count"/>条，其中最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s20-nh"/>（属于路由器<AnswerInput size="medium" questionId="Lab6-p3-s20-router"/>）
    <StepRecordCard 
...
3.  查看R1的路由表；观察可知，到达R2-R8间子网的下一跳IP为<AnswerInput size="large" questionId="Lab6-p3-s20-rt-ip"/>，属于路由器<AnswerInput size="medium" questionId="Lab6-p3-s20-rt-router"/>
    <StepRecordCard 
      questionId="Lab6-p3-s20" 
      title="R1路由表示例" 
      uploadOptions={[
...
4.  在R1上跟踪到达R2-R8间子网的路由，验证路径是否穿越了AS 65003；观察可知，依次经过了路由器<AnswerInput size="medium" questionId="Lab6-p3-s20-tr1"/>、<AnswerInput size="medium" questionId="Lab6-p3-s20-tr2"/>、<AnswerInput size="medium" questionId="Lab6-p3-s20-tr3"/>、<AnswerInput size="medium" questionId="Lab6-p3-s20-tr4"/>
    <StepRecordCard 
      questionId="Lab6-p3-s20" 
      title="路由跟踪示例" 
      uploadOptions={[
...
1.  在R3上的BGP中启用OSPF路由重分发功能（命令：`router bgp [AS-number]`, `redistribute ospf [pid]`）
    ...
    />
2.  查看R3的BGP数据库，标记新增的路由信息；观察可知，新增的路由分别是：<AnswerInput size="large" questionId="Lab6-p3-s21-net1"/>、<AnswerInput size="large" questionId="Lab6-p3-s21-net2"/>、<AnswerInput size="large" questionId="Lab6-p3-s21-net3"/>，因为重分发后，BGP将在AS之间传播OSPF的内部路由信息
    <StepRecordCard 
      questionId="Lab6-p3-s21" 
      title="R3 BGP数据库示例" 
      uploadOptions={[
...
3.  等待一会，在R8上查看BGP数据库，观察AS 65003的内部相关路由信息是否存在；观察可知，AS 65003内部子网的路由有<AnswerInput size="small" questionId="Lab6-p3-s21-r8-count"/>条，其中到达R3的回环口的最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s21-r8-r3"/>，到达R4的回环口的最佳路由的下一跳为<AnswerInput size="large" questionId="Lab6-p3-s21-r8-r4"/>
    <StepRecordCard 
      questionId="Lab6-p3-s21" 
      title="R8 BGP数据库示例" 
      uploadOptions={[
...
在配置过滤策略前，我们先查看当前的状态，查看R7的BGP数据库中PC3所在子网的最佳路由；观察可知到达PC3子网的最佳路由的下一跳是<AnswerInput size="large" questionId="Lab6-p4-s24-r7"/>
...
你会发现R7的BGP数据库中只有1条前往PC3子网的路由，这主要是因为此时R6认为其最佳路径是65007→65008→65002的路径，而R7属于65007，为了避免路由环路，R7不会采纳R6宣告的该路径；而BGP路由器默认只向邻居通告自己认为的最佳路径，因此R6不会向R7宣告另一条路径
...
1.  在R8上创建访问列表（命令：`access-list [id] deny [subnet] [wildcard-mask]`, `access-list [id] permit any`），用于匹配要过滤的路由（注意ACL末尾隐含deny all，所以如果只deny一条，需要显式permit any）
...
5.  查看R7的BGP数据库中PC3所在子网的最佳路由（命令：`show ip bgp [subnet]`）；观察可知，R8上到达PC3子网的最佳路由的下一跳是<AnswerInput size="large" questionId="Lab6-p4-s25-r8"/>，该路由被过滤，没有传递给R7，因此R7上到达PC3子网的最佳路由的下一跳是<AnswerInput size="large" questionId="Lab6-p4-s25-r7"/>，数据不再经过AS 65008了
    <StepRecordCard 
      questionId="Lab6-p4-s25" 
      title="R7 BGP数据库示例" 
      uploadOptions={[
...
| :--- | :--- | :--- | :--- |
| **全球单播 (GUA)** | 余下部分 | 公网IP | 全球唯一互联网地址，运营商通常分配`/48`，企业内部再划分`/64`到各VLAN |
| **唯一本地 (ULA)** | `FC00::/7` | 私网IP | 局域网专用，公网不可路由，推荐使用`FD00::/8`段规划内网，替代旧版Site-Local |
| **站点本地 (Site-Local)** | `FEC0::/10` | 私网IP | 类似于IPv4的私网地址，现在已被IETF废弃，由ULA取代 |
...
* **多播地址**：`FF00::/8`；IPv6取消了广播，改用多播，其中 `FF02::1` 代表链路内所有节点，`FF02::2` 代表链路内所有路由器
...
指定 IPv6 地址时，你**必须**手动指定掩码（如 `ipv6 address [ip]/[mask]`），否则会使用默认的`/64`子网掩码，此时PC会认为目标网络在相同子网，不通过默认网关转发，导致无法正常通信
...
1.  激活R1上的f0/1端口，配置IPv6的site-local地址（命令：`ipv6 address fec0::101:1/112`）；给f2/0口配置IPv6的site-local地址
    ...
    />
2.  查看IPv6接口（命令：`show ipv6 interface`），标记自动分配的link-local地址；观察可知，系统为f0/1端口自动分配的链路本地地址为<AnswerInput size="large" questionId="Lab6-p5-s26-f01"/>，为f2/0端口自动分配的链路本地地址为<AnswerInput size="large" questionId="Lab6-p5-s26-f20"/>
    <StepRecordCard 
      questionId="Lab6-p5-s26" 
      title="R1 IPv6接口示例" 
      uploadOptions={[
...
1.  给R6的f2/0、f0/1端口配置IPv6的site-local地址，查看IPv6接口并标记自动分配的link-local地址；观察可知，系统为f0/1端口自动分配的链路本地地址为<AnswerInput size="large" questionId="Lab6-p5-s27-f01"/>，为f2/0端口自动分配的链路本地地址为<AnswerInput size="large" questionId="Lab6-p5-s27-f20"/>
    <StepRecordCard 
...
3.  宣告直连网络（命令：`network [ipv6-prefix]/[length]`），并互相设置对方为IPv6邻居（命令：`neighbor [ipv6-addr] remote-as [as]`, `neighbor [ipv6-addr] activate`）
    ...
    />
4.  在R6查看IPv6单播邻居信息（命令：`show bgp ipv6 unicast neighbors`）；观察可知与IPv6地址<AnswerInput size="large" questionId="Lab6-p5-s28-r6"/>的邻居状态关系已为Established
    <StepRecordCard 
      questionId="Lab6-p5-s28" 
      title="R6 IPv6邻居信息示例" 
      uploadOptions={[
...
5.  在R1查看IPv6单播邻居信息（命令：`show bgp ipv6 unicast neighbors`）；观察可知与IPv6地址<AnswerInput size="large" questionId="Lab6-p5-s28-r1"/>的邻居状态关系已为Established
    <StepRecordCard 
      questionId="Lab6-p5-s28" 
      title="R1 IPv6邻居信息示例" 
      uploadOptions={[
...
1.  给PC2配置IPv6的site-local地址（命令：`ip [ipv6-address]/[prefix-len]`），由于系统会自动配置链路本地的地址并发现本地链路上的默认路由器，这里我们不需要配置默认路由器
    ...
    />
2.  查看IPv6信息（命令：`show ipv6`），标出链路本地地址及路由器的MAC地址；观察可知，链路本地地址为：<AnswerInput size="large" questionId="Lab6-p5-s29-link"/>，路由器的MAC地址为：<AnswerInput size="large" questionId="Lab6-p5-s29-mac"/>
    <StepRecordCard 
      questionId="Lab6-p5-s29" 
      title="PC2 IPv6配置示例" 
      uploadOptions={[
...
2.  查看IPv6信息（命令：`show ipv6`），标出链路本地地址及路由器的MAC地址；观察可知，链路本地地址为：<AnswerInput size="large" questionId="Lab6-p5-s30-link"/>，路由器的MAC地址为：<AnswerInput size="large" questionId="Lab6-p5-s30-mac"/>
3.  用Ping测试与R6的连通性
...
2.  给PC4配置IPv6地址
3.  用Ping测试PC4和R2的连通性
4.  用Ping测试PC4和PC2的连通性；观察可知，此时由于路由器<AnswerInput size="medium" questionId="Lab6-p5-s32-router"/>没有<AnswerInput size="large" questionId="Lab6-p5-s32-route"/>的IPv6路由，无法Ping通PC2
...
<StepRecordCard 
  ...
  uploadOptions={[
    ...
    { id: 'pc4_pc2_ping', label: '截图PC4→PC2 Ping结果' }
  ]}
/>
...
<StepRecordCard 
  ...
  title="R1隧道配置示例" 
  uploadOptions={[
    { id: 'r1_tunnel_config', label: '记录R1隧道配置', type: 'text', textConfig: { initialContent: "R1(config)#interface Tunnel0\nR1(config-if)#ipv6 address [address]\nR1(config-if)#tunnel source f1/0\nR1(config-if)#tunnel destination [ip]\nR1(config-if)#tunnel mode ipv6ip",codeEditor: true, initialLines: 5 } }
  ]}
>
...
<StepRecordCard 
  ...
  title="R2隧道配置示例" 
  uploadOptions={[
    { id: 'r2_tunnel_config', label: '记录R2隧道配置', type: 'text', textConfig: { initialContent: "R2(config)#interface Tunnel0\nR2(config-if)#ipv6 address [address]2\nR2(config-if)#tunnel source f1/0\nR2(config-if)#tunnel destination [ip]\nR2(config-if)#tunnel mode ipv6ip",codeEditor: true, initialLines: 5 } }
  ]}
>
...
1.  在R1、R2上为对方的IPv6子网设置静态路由（命令：`ipv6 route [ipv6-network] Tunnel [id]`）,下一跳为隧道接口
    <StepRecordCard 
      ...
      title="R1静态路由配置示例" 
      uploadOptions={[
        { id: 'r1_ipv6_static', label: '记录R1静态路由配置', type: 'text', textConfig: {initialContent: "R1(config)# ", codeEditor: true, initialLines: 1} },
...
2.  用Ping测试PC5到PC4之间的连通性；观察可知从路由器<AnswerInput size="medium" questionId="Lab6-p5-s35-router"/>返回没有路由的错误
3.  如果不通，查看R6上的路由信息，思考下为什么；观察可知，R6上没有<AnswerInput size="large" questionId="Lab6-p5-s35-route"/>的路由
...
- 在AS内部两个BGP邻居是否一定要直接连接？如果不直接连接，它们之间是如何获得到达对方的路由的？需要和OSPF那样建立虚链路吗？
  <AnswerInput size="exlarge" questionId="Lab6-q1"/>
- 默认情况下，BGP根据什么条件决定最佳路由？
  <AnswerInput size="exlarge" questionId="Lab6-q2"/>
- 为什么未启用同步时，R1选择AS65003作为到达R2的转发路径时，R3和R5的路由表都存在去往R2的路由，但实际却不能Ping通？
  <AnswerInput size="exlarge" questionId="Lab6-q3"/>
- 为什么未启用路由重分发时，R4没有外部网络的路由？
  <AnswerInput size="exlarge" questionId="Lab6-q4"/>
- 为什么PC可以不设置IPv6的默认路由器？路由器可以吗？
  <AnswerInput size="exlarge" questionId="Lab6-q5"/>
- R1和R2两边的IPv6网络是采用什么技术通过IPv4的网络进行通信的？R6的IPv6网络又是如何实现与R2的IPv6网络通信的？
  <AnswerInput size="exlarge" questionId="Lab6-q6"/>

## 4 讨论与心得

在完成本实验后，你可能会有很多待解答的问题，你可以把它们记在这里，接下来的学习中，你也许会逐渨得到答案的，同时也可以让老师了解到你有哪些困惑，老师在课堂可以安排针对性地解惑。等到课程结束后，你再回头看看这些问题时你或许会有不同的见解：

<AnswerInput size="exlarge" questionId="lab6-question"/>

在实验过程中你可能会遇到的困难，并得到了宝贵的经验教训，请把它们记录下来，提供给其他人参考吧：

<AnswerInput size="exlarge" questionId="lab6-experience"/>

你对本实验安排有哪些更好的建议呢？欢迎献计献策：

<AnswerInput size="exlarge" questionId="lab6-suggestion"/>

## 5 常见问题

* R6 与 R2 采用“隧道 + 双栈”混合互联时，流量转发优先级如何确定？优先走隧道还是原生 IPv6？

  流量走哪条路完全取决于路由表的选路原则，路由器不会因为物理接口或虚拟接口的差异而产生天生的优先级；首先遵循**最长匹配原则**，如果原生链路和隧道都通过动态路由协议学习到了去往同一目的地的路由，且掩码长度一致，路由器会比较两种协议的**管理距离**，例如原生 IPv6 跑的是 OSPF（管理距离 110）而隧道配置了静态路由（管理距离 1），那么流量会走隧道；如果协议也相同，则比较**开销值**，通常隧道会增加额外的封装开销或被人工设置较高的 Cost 以作为备用链路，因此在默认配置下，如果通过相同的动态路由协议学习，原生链路往往因开销更小而被优选

* 实际网络中如何配置 BGP 的 20 多个属性以满足业务需求？

  实际网络中，我们极少会同时操作所有属性，而更多通过 **Route-Map** 工具根据流量方向有选择地修改关键属性；对于**出站流量**（从本 AS 出去），通常在入口方向修改 Local Preference 属性，因为它的优先级最高且在 AS 内通用；对于**入站流量**（外部数据进入本 AS），通常使用 AS-Path Prepending（人为增加 AS 号重复次数）来让路径看起来更远，或者与对端协商使用 MED 属性；配置流程通常是先用前缀列表或 ACL 抓取特定的路由条目，然后在 Route-Map 中 `match` 这些条目并 `set` 相应的属性值，最后挂载到 BGP 邻居的 `in` 或 `out` 方向上

* 在多个 AS 间进行重分发时，如何防止路由失真和环路？

  防止环路最核心的手段是使用**路由标记**，当路由从 AS A 重分发进 AS B 时，给这些路由打上一个特定的 Tag 值，当 AS B 试图将路由再重分发回 AS A 或其他相关联的 AS 时，配置策略拒绝所有携带该 Tag 的路由，从而切断环路；防止路由失真（即次优路径）则需要手动调整**管理距离**，因为不同协议的 Metric 计算方式不同（带宽 vs 跳数），重分发后 Metric 丢失，可能导致路由器误认为绕远路的外部重分发路由比内部直连路由更优，通过调高外部路由的管理距离可以强制设备优先信任内部路径

* BGP 既然需要同步、重分发等机制解决常见问题，为什么不直接整合到默认机制中？

  BGP 的设计哲学是**稳定性**和**策略控制**高于连通性，这与 IGP（如 OSPF）追求快速互通截然不同；**关于同步**，实际上现代路由器（如 Cisco IOS 12.2 以后）默认已经关闭了同步功能，因为现网大多采用全互联 IBGP 或路由反射器，同步机制已经过时；**关于重分发**，BGP 承载着全球几十万条互联网路由，如果默认允许自动重分发，任何一个内部网络的微小抖动都会被扩散到全球 BGP 网络中引发震荡，因此 BGP 强制要求管理员必须显式地宣告网络或配置策略，以确保只有稳定、合规的路由才会被通告出去
