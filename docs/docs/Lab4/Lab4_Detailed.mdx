...
<DeadlineProgress start={'2025-11-11 18:50:00'} end={'2025-11-25 23:59:59'}/>
<GridContainer>
    ...
    <FileCard file_type={'pdf'} name={'Lab4 静态路由配置 实验课件（修改）'} size={'2101192'} link={require('@site/assets/slides/Lab4 静态路由配置.pdf').default} />
...
一台路由器在没有任何静态或动态路由配置的情况下，其路由表中只包含与它**直接相连**的网络信息，这些路由是路由器通过配置接口IP地址自动生成的，通常用字母`C`（Connected，代表直连的网段，如接口所属的子网）或`L`（Local，较新的IOS版本中存在，代表路由器在直连网段上配置的具体IP地址）标识
...
:::tip
`network`命令在这里的作用是声明该地址池可以从哪个网段中分配IP地址，而不是配置一条路由
:::
...
由于DHCP的Discover报文是广播包，无法被分隔广播域的路由器转发，路由器的每个接口所连接的网段如果需要DHCP服务都必须有一个独立的地址池为其服务，因此我们需要用相似的方式，在R4上为f0/1接口所连接的子网（`172.16.1.0/24`）配置第二个DHCP地址池

<StepRecordCard 
  questionId="Lab4-s10" 
  title="第二个DHCP池配置命令" 
  uploadOptions={[
    { id: 'config_cmd', label: '记录配置命令', type: 'text', textConfig: {initialContent: "R4(config)# ip dhcp pool 2\nR4(dhcp-config)# network 172.16.1.0 /24\nR4(dhcp-config)# default-router 172.16.1.1", codeEditor: true, initialLines: 3} }
  ]}
/>
...
**串口**（**Serial Port**）是一种常用于广域网（WAN）连接的接口类型，与以太网不同，它通常用于点对点的长距离连接；在广域网场景中，串口常用于连接地理位置分散的路由器，例如通过电信运营商提供的专线（如T1/E1线路）将不同城市的分支机构网络连接起来；与局域网中常见的以太网接口相比，串口的速率通常较低（如64kbps或1.544Mbps），远低于现代以太网动辄百兆、千兆的速率，其设计侧重于长距离的稳定传输而非高速通信；如今在大多数广域网场景中，这类传统的低速串口连接已基本被速度更快、更可靠的光纤所取代

![串口接口示例](img/1761331554577.png)

**HDLC (High-Level Data Link Control)** 是Cisco路由器串口上默认的数据链路层封装协议，它是一个简单的、低开销的协议，适用于连接两台思科设备的点对点链路

**时钟速率**（**Clock Rate**）：在串行通信中，数据是一位一位地传输的，为了让接收端能够正确地同步和解码这些比特流，需要一个时钟信号来规定传输速率，在物理连接中，提供时钟信号的一端被称为**DCE (Data Communications Equipment，数据通信设备)**，而接收时钟信号进行同步的一端被称为**DTE (Data Terminal Equipment，数据终端设备)**，`clock rate` 命令**必须在DCE端配置**；实验中我们选取的`64000`是一个串行链路中常用的标准速率（64kbps），在模拟环境中，这个值的选取一般不会影响实验结果

</details>

1.  分别进入R1和R2的串口接口配置模式
2.  配置数据链路层协议为HDLC（命令：`encapsulation hdlc`）
3.  选取其中任意一台路由器作为DCE端，在其上设置时钟速率（命令：`clock rate 64000`）
4.  为两个接口配置IP地址并激活接口
    <StepRecordCard 
      questionId="Lab4-s14" 
      title="R1配置命令示例" 
...
静态路由生效有一个前提条件：通往下一条的接口必须是UP状态，在我们没有关闭主链路的接口时，这条备用路由不会出现在路由表中，只有当主路由（AD值更小的那条）因为链路故障等原因从路由表中消失时，它才会“浮现”出来，被激活并加入路由表，从而实现链路的自动备份和切换

</details>

1.  在R1和R4上，为对方的PC所在子网添加一条备用路由，选择**经过R2的串口线路**作为下一跳路径，并将管理距离（路由距离）设置为30（命令：`ip route 目标网络 子网掩码 下一跳地址 距离`）；这里选择30是为了和默认的静态路由（AD为1）拉开显著差距，任何大于静态路由AD的整数值在此处都可以实现浮动路由的效果
    <StepRecordCard 
      questionId="Lab4-s21" 
      title="备用路由配置命令" 
      uploadOptions={[
...
`ping`命令的返回包需要有回到源IP的路由才能正确返回，而`ping` 命令默认使用出接口（前往目标IP对应下一跳IP的接口）的IP地址作为源地址，当我们想检验网络中的其他路由器是否有回到特定源IP的路由时，这种默认行为无法满足我们的需求；而通过 `ping [目标IP] source [源IP或接口]` 的方式，我们可以指定一个不同的源地址来发起测试，这对于验证特定网络路径的双向连通性非常有用
...
<details>
<summary>背景知识：Access Control List 访问控制列表</summary>

ACL (Access Control List)即访问控制列表，在网络中承担“门卫”的作用，路由器可以利用它来识别和过滤进出的数据包，从而实现对网络流量的精细化控制

想象一下，网络就像一个繁忙的十字路口，车流（数据包）来来往往，但并非所有车辆都应该被允许通过，有些可能需要被引导到特定车道，有些则可能因为安全原因需要被拦截；路由器要正确地做出判断，就需要依靠预先设定好的一系列规则，也就是访问控制列表

ACL的本质是一张有序的规则清单，每一条规则都是一个 permit（允许）或 deny（拒绝）的声明，后面跟着具体的匹配条件，例如数据包的源地址、目标地址、协议类型（TCP/UDP）甚至是端口号；当一个数据包到达路由器接口时，路由器会像门卫核对访客名单一样，从上到下逐一检查ACL中的规则；一旦找到匹配的规则，路由器就会立即执行相应的允许或拒绝动作，并且不再继续检查后续规则；如果数据包不匹配任何规则，通常会触发一条隐藏在列表末尾的、默认的“全部拒绝”规则

在NAT的场景中，ACL发挥的作用主要是筛选而非拦截；它并非为了拦截流量，而是为了识别出哪些流量需要被NAT转换；例如，我们可以创建一条ACL规则，专门“挑出”所有源自内部私有网段（如 192.168.0.0/24）的数据包，然后NAT服务就可以根据这条ACL的“指引”，只对这些被选中的数据包进行地址转换，而忽略其他无关的流量

</details>

1.  在R5上配置NAT服务，由于NAT服务是有方向性的，我们定义fa0/1为**内部接口**，fa0/0为**外部接口**，这样数据从fa0/1流入，从fa0/0流出时就会触发内部→外部的地址转换，反之亦然
    :::danger 实验报告模板中，内部/外部接口写反，请按实验文档要求，定义f0/1接口为内部接口，定义f0/0接口为外部接口
    :::
2.  创建ACL以允许Private区域（`192.168.0.0/24`）的流量
3.  设置NAT规则，将内部流量的源地址转换为R5的fa0/0接口地址；请注意，Access List配置时采用的是通配符掩码，0表示需要精确匹配的位，因此应为子网掩码的取反，例如对于`192.168.0.0/24`，通配符掩码应为`0.0.0.255`，路由器将只严格检查0对应位是否匹配；由于我们这里只要转换一个内部网络的源地址，因此只需要配置1号访问列表即可
    <StepRecordCard 
      questionId="Lab4-s25" 
      title="NAT配置命令示例" 
      uploadOptions={[
...
1. 在各路由器上增加静态路由信息，使得PC6能够与Zone1、Zone2、Guest Zone的PC机通信，如果某个路由器上不需要增加静态路由信息，配置命令填`-`即可
    :::tip 提示
    R5是一个典型的末梢路由器（Stub Router），它只有一个出口（通往R3）可以访问外部所有网络，对于这样的路由器，我们不需要为每个外部网络都写一条静态路由，只需要配置默认路由（命令：`ip route 0.0.0.0 0.0.0.0 [默认路由器IP地址]`）将所有不知道怎么转发的都交给R3就能实现通信；另外需要注意的是，Private Zone对其他区域是不可见的（其源IP地址都被“伪装”成了R5的外部接口IP地址），所以在外部路由器上是不需要也不应该为其添加路由的（只需要添加R3-R5之间的子网）
    :::
    <StepRecordCard 
      questionId="Lab4-s26" 
      title="各路由器配置命令" 
...
1.  将R2的f0/0接口连接到GNS3中的`Cloud`设备的eth0接口（对应VMWare中仅主机模式的虚拟网络适配器，如果你在Linux环境无法通过该接口通信，请参考[Linux使用提示](/docs/GNS3/VM-VMware#3-配置gns3虚拟机)）），该设备与电脑主机的某个虚拟网卡处于同一子网，IP地址动态分配
...
1.  寻找一个无需认证的真实网络环境（如手机热点），将你的电脑和另一台设备（主机H）接入该网络
    ...
    :::
2.  在GNS3中，将R2的f0/0接口改连到Cloud设备的eth2接口（如果你在Linux使用非VMWare进行虚拟化且无法找到该接口，请参考[Linux使用提示](/docs/GNS3/client-usage#21-网络云)）
...
* 路由器的接口为什么会出现：FastEthernet0/1 is up, line protocol is down的状态？
  <AnswerInput size="exlarge" questionId="Lab4-q1"/>
* 路由起什么作用？什么是静态路由？
  <AnswerInput size="exlarge" questionId="Lab4-q2"/>
* 需要为每个PC的IP地址添加路由，还是只需要为其网络地址添加路由？
  <AnswerInput size="exlarge" questionId="Lab4-q3"/>
* 添加静态路由时，下一跳地址是填写本路由器的端口地址，还是对方路由器的端口地址？或者是目的地网络的路由器端口地址？
  <AnswerInput size="exlarge" questionId="Lab4-q4"/>
* 什么是默认路由？添加默认路由的命令格式是什么？
  <AnswerInput size="exlarge" questionId="Lab4-q5"/>
* 在同一个局域网内的2台PC机，IP地址分别为10.0.0.x/24和10.0.1.x/24，都属于VLAN1，一开始不能互相Ping通，为什么把子网掩码长度从24位变成16位，就通了？
  <AnswerInput size="exlarge" questionId="Lab4-q6"/>
* 如果仅仅是为了让不同区域内的PC之间能够互相Ping通，在设置静态路由时，路由器之间互联的子网是否全部都要加入到所有路由器的路由表中？为什么？
  <AnswerInput size="exlarge" questionId="Lab4-q7"/>

## 4 讨论与心得

在完成本实验后，你可能会有很多待解答的问题，你可以把它们记在这里，接下来的学习中，你也许会逐渐得到答案的，同时也可以让老师了解到你有哪些困惑，老师在课堂可以安排针对性地解惑。等到课程结束后，你再回头看看这些问题时你或许会有不同的见解：

<AnswerInput size="exlarge" questionId="lab4-question"/>

在实验过程中你可能会遇到的困难，并得到了宝贵的经验教训，请把它们记录下来，提供给其他人参考吧：

<AnswerInput size="exlarge" questionId="lab4-experience"/>

你对本实验安排有哪些更好的建议呢？欢迎献计献策：

<AnswerInput size="exlarge" questionId="lab4-suggestion"/>

## 5 常见问题

* **GNS3 服务器在实验里起到的作用是什么？**

GNS3 服务端是整个模拟环境的后端引擎；客户端主要负责图形界面的展示，可在客户端进行拓扑图的构建和编辑，而服务器端则负责模拟客户端设计拓扑图的设备与链路，加载和执行路由器、防火墙的操作系统镜像（如 IOS、IOU 或 QEMU 虚拟机），所有的报文转发计算、CPU 与内存资源的消耗实际上都发生在服务端上

* **当默认路由与更精确的静态路由同时存在时，若默认路由下一跳故障，路由器会自动切换还是丢弃数据包？**

这取决于数据包具体匹配到了哪条路由；路由器遵循最长匹配原则，如果数据包的目的地址匹配到了那条更精确的静态路由，那么无论默认路由是否故障，数据包都会正常转发；但如果数据包仅能匹配默认路由，一旦默认路由因下一跳故障而从路由表中消失，路由器将找不到去往该目的地的路径，从而直接丢弃数据包并可能返回目标不可达消息

* **配置 NAT 时，如何选择 ip nat inside/outside 接口和匹配的 ACL？**

**接口选择**遵循数据流向的物理边界，连接本地局域网（LAN）的接口配置为 `ip nat inside`，因为这里是数据的源头，连接运营商（ISP）或公网的接口配置为 `ip nat outside`，因为这里是数据转换后出去的出口；**ACL 选择**则用于定义“谁有资格被转换”，通常你需要编写一个标准 ACL，使用 `permit` 语句匹配内部局域网的源 IP 地址段，任何未被该 ACL 匹配的流量将不会触发 NAT 操作，而是直接透传或丢弃

* **浮动静态路由中，若两条路径管理距离（AD）相同，路由器如何选择？是否负载均衡？**

如果两条指向同一目的地的静态路由具有相同的管理距离，那么它们就不再是主备关系的“浮动静态路由”，而是形成了等价路由（ECMP）；此时路由器会将这两条路径同时加入路由表，并对流量进行负载均衡，数据包会根据算法（如基于流或基于包）被分配到这两条链路上同时传输，从而利用双倍的带宽

* **外网能否 ping 通 NAT 内网机器？**

NAT有多种类型可用，在最常用的动态 NAT 或 PAT（端口复用）模式下，外网确实无法主动 Ping 通内网主机，这是因为 NAT 转换表项通常是由内网发往外网的流量触发建立的，如果外网直接发起请求，路由器在 NAT 表中找不到对应的映射记录，不知道该把数据包转发给内网的哪台机器，因此会直接丢弃；但在配置了静态 NAT（Static NAT）将内网 IP 一对一映射/配置了端口映射时可以Ping通内网设备
