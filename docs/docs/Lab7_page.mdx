...
description: 计算机网络实验七（Lab7）指导手册，内容为基于Socket接口实现自定义协议通信。本实验旨在加深对网络应用和通信原理的理解，涵盖自定义协议设计（包括数据编解码与错误处理）、并发客户端连接处理，并提升现代化开发工具的使用技能。
---
...
<DeadlineProgress start={'2025-09-28 18:50:00'} end={'2025-12-30 23:59:59'}/>
<GridContainer>
...
<ApiTestRunner header={"测试1 服务端连接与发送测试"} testUri={"/test/lab7/standalone"}/>
...
<LinkCard title="std::thread - cppreference.com" url="https://zh.cppreference.com/w/cpp/thread/thread">
...
<ApiTestRunner header={"测试3 服务端多线程测试"} testUri={"/test/lab7/threaded"} multiThread/>
...
实现通信协议时，禁止照搬/抄袭“前人智慧”，使用定长数据包而不进行合理的协议将影响评分，并使后续实验完成产生困难
...
在按照实验报告要求测试服务器断连等情况时，如果无论如何均无法避免服务端向客户端发送下线告知消息，请考虑暂时注释本部分代码，以模拟异常退出/断联状态，如仍然无法模拟相应情况，请考虑使用实体机进行相应步骤测试
:::
...
* **结构化绑定（`C++17`开始可用）** 对于含多元素容器，如`pair[int, string]`，可直接将其解构并绑定到指定的局部变量上，这使得遍历`map`等时可以避免较模糊的`.first`/`.second`
  ```cpp
  ...
  ```
* **输出格式化** 引入`iomanip`库，可指定输出每个元素所占宽度/对齐/空白填充方式等，实现格式化输出
...
* **退出功能** 
  如已和服务器建立  连接，关闭Socket连接（析构封装Socket）；退出程序
...
这里指的是物理意义上的断网，因此建议你在两台电脑完成，你可以和室友/同组同学一起完成，使用手机作为热点，两台电脑连接到热点；服务端如果在WSL上运行，请打开WSL Settings将服务端所在的WSL的网络从NAT模式改为Mirrored模式，并重启WSL（在Windows命令行/PowerShell中执行`wsl --shutdown`，随后重新启动相应WSL）；重启后，在WSL内输入`ip addr`查看WSL获取到的热点IP，客户端使用该IP即可与服务端建立连接
...
为了提高传输效率，TCP协议栈会主动合并多次向相同设备传输的小数据包，因此你收到的数据包数量小于获取时间次数是正常的；如果电脑性能较差/与同组同学电脑间网络连接不佳，也可能因大量重传而导致你收到的数据包数量小于获取时间次数；请结合Wireshark抓包结果进行具体分析

由于通常获取100次时间速度非常快，你可能来不及同时开始，为此可以使用以下bash测试脚本(Windows用户请用Git Bash运行)触发同时获取，**你需要将脚本内头部的配置参数改为你自己的**：

``` c
#!/bin/bash

CLIENT_PATH="./client" # 客户端程序路径
INPUT_CONN_OPT="1"     # 连接服务器的选项
INPUT_IP="127.0.0.1"   # 服务器 IP 地址
INPUT_PORT="8080"      # 服务器 端口号
INPUT_ACTION_OPT="2"   # 获取100次服务器时间的选项
INPUT_EXIT_OPT="0"     # 退出程序的选项

mkfifo /tmp/sync_barrier
exec 3<>/tmp/sync_barrier # 以读写模式打开打开 FIFO 描述符3

for i in 1 2; do
    { 
      echo "$INPUT_CONN_OPT" # 输入选项
      echo "$INPUT_IP"       # 输入IP
      echo "$INPUT_PORT"     # 输入端口
      read -u 3 line 
...
不建议将获取时间次数大幅提高，过大的获取时间次数可能在短时间内导致服务端、客户端TCP协议栈的发送环形缓冲区溢出，从而导致数据包未被发出即丢失（请注意：此时该数据包尚未轮到被TCP发送即被覆盖，因此TCP协议栈完全不会意识到存在该数据包，也不存在相应的超时重传）
