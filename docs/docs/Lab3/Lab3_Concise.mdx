---
sidebar_position: 2
title: Lab3 使用三层交换机组网 精简版
description: 计算机网络实验三（Lab3）使用三层交换机组网的精简版指导，重点介绍通过单臂路由器和三层交换机实现 VLAN 间通信的两种方法。文档包含实验目的、内容、所需设备及相关操作提示。
---
...
1. 将2台PC和一台路由器都连接到一台二层交换机，在交换机上增加1个VLAN，并使得2台PC所连端口分别属于2个VLAN。给2个PC分配不同子网的IP地址。
   ...
   ![单臂路由实验拓扑图](img/image2.png)

2. 验证两个PC之间能否Ping通（不同的VLAN之间不通）

   截图：Ping结果

   ![不同VLAN间Ping测试结果](img/image3.png)



3. 将二层交换机和路由器连接的端口配置成VLAN Trunk模式（请参考实验2 进行配置，无需截图）。

   例：配置命令（示例为GNS3模拟的配置截图，使用实际设备请参考实验2）

   ![交换机Trunk模式配置命令示例](img/image4.png)

   例：配置后的结果（示例为GNS3模拟的结果截图，使用实际设备请参考实验2）

   ![Trunk模式配置结果验证](img/image5.png)
...
6. 测试2台PC能否Ping通各自的路由器子接口地址
   ...
   ![路由器子接口配置命令示例](img/image8.png)

   ![路由器子接口配置结果验证](img/image9.png)
...
7. 测试2台PC能否互相Ping通
   ...
   ![单臂路由配置完成后的Ping测试结果](img/image10.png)
...
1. 将第一部分的路由器移除后，将二层交换机和一台三层交换机连接，并新增2台PC（PC3、PC4）直接连接到三层交换机，为PC3、PC4分配所在VLAN内的合适IP地址，并在图中标记各设备的IP地址和VLAN

   拓扑图参考，请替换成实际使用的：

   ![三层交换机实验拓扑图](img/image13.png)
...
2. 在三层交换机上增加1个VLAN，并使得2台PC所连端口分别属于2个VLAN。
   ...
   ![三层交换机VLAN配置结果截图](img/image14.png)
...
6. 测试PC3、PC4能否Ping通各自的VLAN接口地址
   ...
   ![PC3 Ping VLAN接口结果截图](img/image17.png)

   ![PC4 Ping VLAN接口结果截图](img/image18.png)
...
7. 测试PC3、PC4能否互相Ping通。
   ...
   ![PC3 与 PC4 互相 Ping 测试结果截图](img/image19.png)
...
8. 测试不同交换机上属于不同VLAN的PC间的连通性（如PC1-\>PC4, PC2-\>PC3）
    ...
    * PC1→PC4
      ...
      ![PC1 Ping PC4 测试结果截图](img/image20.png)

    * PC2→PC3

      ![PC2 Ping PC3 测试结果截图](img/image21.png)
...
10. 再次测试之前不通的PC间的连通性。
    ...
    * PC2→PC3
      ...
      ![PC2 再次 Ping PC3 的测试结果截图（Trunk 模式启用后）](img/image22.png)
