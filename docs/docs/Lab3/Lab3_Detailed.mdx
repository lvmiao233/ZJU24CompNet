...
<DeadlineProgress start={'2025-10-28 18:50:00'} end={'2025-11-11 23:59:59'}/>
<GridContainer>
...
:::important 提示
以下实验记录均需结合**屏幕截图**，进行文字标注和描述，图片应大小合适、关键部分清晰可见，**严禁手机拍屏**，可直接在图片上进行标注，也可以单独用文本进行描述
...
二层交换机的交换虚拟接口IP没有任何网关转发能力（能够进行三层转发的交换机即可称为三层交换机，因此二层交换机一定不能进行三层转发，也因此不具有作为默认网关的能力）

默认网关必须是一个确实存在的端口/虚拟接口IP，在本步骤中，VLAN1的两台设备均需要配置默认网关为三层交换机VLAN 1的交换虚拟接口IP，VLAN2的两台设备需要配置默认网关为三层交换机VLAN 2的交换虚拟接口IP；如果你为三层交换机的交换虚拟接口配置的IP与PC1/2原有默认网关不同，请你调整PC1/2的默认网关配置；如果配置任何其他不存在的IP，则PC在与非本网络进行通信时均无法找到默认网关进行转发（因为你配置了不存在的IP，不可能找到对应的设备）
...
* 为什么路由器的端口可以配置IP地址，而三层交换机的端口跟二层交换机一样不能配置IP地址？
   <AnswerInput size="exlarge" questionId="Lab3-q1"/>
* 本实验中为什么要用子接口？有什么好处？使用物理接口可以吗？
   <AnswerInput size="exlarge" questionId="Lab3-q2"/>
* 直连三层交换机的PC的默认路由器地址应该设为什么？
   <AnswerInput size="exlarge" questionId="Lab3-q3"/>
* 三层交换机和二层交换机互联时，连在二层交换机上VLAN2的PC为什么Ping不通连在三层交换机上VLAN 1的PC？
   <AnswerInput size="exlarge" questionId="Lab3-q4"/>
* Ping测试时，为什么一开始有几次不通，后面又通了？
   <AnswerInput size="exlarge" questionId="Lab3-q5"/>
* 既然路由器可以实现VLAN间数据交换，为何还要设计三层交换机呢？
   <AnswerInput size="exlarge" questionId="Lab3-q6"/>

## 4 讨论与心得

在完成本实验后，你可能会有很多待解答的问题，你可以把它们记在这里，接下来的学习中，你也许会逐渐得到答案的，同时也可以让老师了解到你有哪些困惑，老师在课堂可以安排针对性地解惑。等到课程结束后，你再回头看看这些问题时你或许会有不同的见解：

<AnswerInput size="exlarge" questionId="lab3-question"/>

在实验过程中你可能会遇到的困难，并得到了宝贵的经验教训，请把它们记录下来，提供给其他人参考吧：

<AnswerInput size="exlarge" questionId="lab3-experience"/>

你对本实验安排有哪些更好的建议呢？欢迎献计献策：

<AnswerInput size="exlarge" questionId="lab3-suggestion"/>

## 5 常见问题

* **ASIC 芯片是如何设计的？为什么转发数据帧的性能显著高于通用芯片？**

  ASIC 全称为专用集成电路，为网络交换需求设计的ASIC能够在转发性能上碾压通用 CPU，这种效率的差异主要来源于：

  * **硬件固化逻辑**：通用 CPU 处理数据包需要经过“取指、译码、执行”等繁琐的软件指令周期，每次转发都需要消耗 CPU 时钟中断；而 ASIC 将查表、校验、改写帧头等转发逻辑直接刻录在芯片的物理电路中，数据进入芯片后如同流水线上的工件，由硬件电路直接并行处理，不需要操作系统的干预
  * **TCAM 查表**：交换机 ASIC 内部通常集成了 TCAM 存储器，它能够在一个时钟周期内同时对比成千上万条表项，无论路由表或 MAC 地址表有多大，查询时间都是固定的常数（O(1)），这就实现了线速转发，端口进多少数据就能处理多少数据，不会因为表项增多而降速

* **STP 为了防止环路会禁用链路，能否将多条物理链路捆绑成一条逻辑链路使用，以提供更好的冗余和更高的带宽？**

  这是可行的，**EtherChannel** / **链路聚合** 等技术就尝试实现了这样的特性，你可以将多条物理链路（通常是2到8条）捆绑在一起，在逻辑上形成一个聚合接口（Eth-Trunk），STP 会将这个聚合组视为一条唯一的逻辑链路，因此不会阻塞其中的物理线路，同时实现了带宽的线性叠加与物理级的冗余


* **如果有很多交换机，是否需要在每一台上面创建所有 VLAN？有没有自动设置的办法？**

  确实不需要在每一台交换机上重复敲命令，业界有成熟的协议来解决 VLAN 信息同步的问题：

  * **VTP 协议**：Cisco私有的 VLAN 中继协议，只需要在一台被配置为 Server 模式的交换机上创建、删除或重命名 VLAN，这些配置信息就会通过 Trunk 链路自动传播到域内所有 Client 模式的交换机上，实现单点配置，全网生效
  * **GVRP 或 MVRP**：非Cisco设备或多厂商混合环境，可以使用基于 IEEE 标准的 GVRP 或其升级版 MVRP，同样能实现 VLAN 属性的动态注册和注销

* **`encapsulation dot1Q 1 native` 命令中出现的 `native` 是什么意思？**

  关键字 `native` 定义了本征 VLAN 的行为规则：在 802.1Q 协议的 Trunk 链路上，绝大多数 VLAN 的数据帧都会被加上 Tag 标记，唯独属于 Native VLAN 的数据帧会以原始以太网帧（不带 Tag）的形式传输；**发送时**，交换机如果发现数据帧属于 Native VLAN，就会剥离标签直接发送；**接收时**，交换机如果收到一个没有携带任何标签的数据帧，就会默认将其归入 Native VLAN 进行处理

  默认情况下 VLAN 1 是 Native VLAN，这个设计最初是为了兼容那些不支持 VLAN Tag 的老旧设备（如集线器），确保管理流量或控制协议（如 CDP、STP BPDU）能在不同设备间正常传递

## 6 导出实验报告

<ExportButton templatePath={require('@site/assets/base_report/Lab3.zip').default} labName="Lab3" labId="lab3-" allowTeammate={true} />
