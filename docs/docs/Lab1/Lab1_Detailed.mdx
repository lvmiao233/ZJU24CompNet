...
<LinkCard title={'Wireshark安装与使用'} url={'/docs/category/wireshark%E5%AE%89%E8%A3%85%E4%B8%8E%E4%BD%BF%E7%94%A8'} icon="https://www.wireshark.org/assets/icons/wireshark-fin@2x.png">在开始实验之前，请务必参考该文档，在你的电脑上成功安装 Wireshark 并了解其基本界面与操作。</LinkCard>
<br/>
...
本部分，我们将捕获设备上进出的数据包，并从中筛选出我们分析所需的关键部分，为后续更深度的分析和理解做好准备


<TaskCard number={1} needScreenshot={false} title="运行Wireshark软件，开始捕获数据包，观察出现的协议">
<details>
<summary>深入理解：网络协议——设备间的“通用语言”</summary>

想象一下，网络中的设备（你的电脑、服务器、路由器等）就像来自世界各地的人。为了能够顺畅交流，他们需要使用一种通用的语言。网络协议（Protocol）就是这套“通用语言”。

实际上，网络通信的复杂性远超日常对话，因此需要一整套分工明确的语言体系。例如：

*   **HTTP (HyperText Transfer Protocol)**: 用于浏览器和服务器之间传输网页内容的“网页语言”。
*   **DNS (Domain Name System)**: 当你输入网址时，负责将网站域名“翻译”成 IP 地址的“问路语言”。
*   **TCP (Transmission Control Protocol)**: 确保数据能够稳定、可靠传输的“快递签收语言”。

网络中的协议成百上千，共同构成了支撑互联网运行的基石。
</details>
...
5.  点击左上角的红色方块按钮 **停止捕获**
6.  在协议（Protocol）列，捕获到的数据包包含的协议有：<ModernInput size="large"/>（请记录至少5个）
...
> 1.  首先，你会把信纸装进一个信封，写上收信人和寄信人的端口号等信息。这个信封就是 **传输层 (TCP/UDP)**
> 2.  接着，为了让信件能在巨大的城市网络中找到正确的街道，邮局会把你的信件再放进一个更大的快递信封，上面写着最终的家庭住址和寄件人地址。这个大信封就是 **网络层 (IP)**，地址就是 IP 地址
> 3.  最后，为了让快递员能在具体的某个小区或楼栋里找到你，需要一个更具体的门牌号。快递包裹会被贴上一个包含这个门牌号（MAC地址）的快递单。这个最终的包裹就是 **数据链路层 (Ethernet)**
> 4.  这个包裹最终通过物理的传输介质（**物理层**，如网线、光纤、无线电波）发送出去

这个从上到下，层层包装的过程，我们称之为 **数据封装 (Data Encapsulation)**。Wireshark 最强大的功能之一，就是能为我们清晰地展示出这个“层层包裹”的结构。
</details>
...
3.  观察主界面下方的分组详情面板，你会看到类似下面这样的分层结构：
    ...
    *   应用层协议
4.  分析：
    * 这个数据包有<ModernInput size="small"/>层
    * 最高层的协议是<ModernInput/>
    * 从Ethernet开始往上，各层协议的名字分别是：<ModernInput size="large"/>
...
<details>
<summary>初步理解：信息过载与过滤器的作用</summary>
...
*   **捕获过滤器 (Capture Filter)**: **在捕获开始前设置，捕获开始后不可改变**，它告诉 Wireshark “弱水三千，只取一瓢”，只有符合规则的数据包才会被 Wireshark 记录下来，其他的数据包则会被**直接丢弃**
    ...
    :::note 提示
    为了实现捕获过滤以尽可能减少内核态向用户态的传输，Linux引入了eBPF（扩展伯克利包过滤器），这一为实现动态调整内核内包过滤逻辑的技术意外开辟了内核可编程化的一条可行技术路线，让我们能在运行时低开销、高灵活地观测与改变内核运行，如果你有相关的需求，不妨了解一下
    :::
...
**记录数据：** 使用的过滤器：<ModernInput/>，希望显示的协议类型：<ModernInput/>
...
1. 在显示过滤器输入框中，输入 `ip.addr == x.x.x.x`，其中 `x.x.x.x` 替换为你感兴趣的具体IP地址。
2. 按下回车，观察筛选结果。
...
4.  **分析捕获到的相关协议数据包**：
    ...
    *   Ping IP地址时，捕获到的**相关**数据包的协议有：<ModernInput/>
    *   Ping 域名时，捕获到的**相关**数据包的协议有：<ModernInput/>
    *   组成ICMP数据包的各层协议有<ModernInput size="large"/>
...
2.  **执行 `tracert` 命令并分析**：
    ...
    *   分析：跟踪路由使用的数据包协议类型是<ModernInput/>，构成数据包的几层协议分别是<ModernInput size="large"/>
3.  **分析请求包**：观察并记录请求包（请对数据包**列表**截图）中IP协议层的TTL字段变化规律，第一个请求的TTL等于<ModernInput size="small"/>，同样TTL的请求连续发送了<ModernInput size="small"/>个，然后每次TTL增加了<ModernInput size="small"/>，最后一个请求的TTL等于<ModernInput size="small"/>
   
    **截图示例**：

    ![请求包列表](img/1.5.jpg)

4.  **分析响应包**：观察并记录响应包的信息（截图第一组、最后一组，并分别标记ICMP层的类型字段），第一组响应包的发送者IP是：<ModernInput/>；最后一组响应包的发送者IP是：<ModernInput/>
...
4.  **分析捕获结果**：
    * 组成捕获到的DNS协议数据包的各层协议是：<ModernInput size="large" />，选取一个DNS查询数据包截图并标记源/目标IP地址、源/目标端口

        **截图示例：**

        ![DNS数据包截图](img/1_clip_image0021.jpg)

    * 组成捕获到的HTTP协议数据包的各层协议是：<ModernInput size="large" />，选取一个HTTP请求数据包截图并标记源/目标IP地址、源/目标端口
...
为了打开网页，浏览器查询的域名有：<ModernInput size="large" />
...
使用显示过滤器`tcp.stream eq X`，让X从0开始变化，直到没有数据，分析浏览器为了获取网页数据总共建立了<ModernInput size="small" />个连接（一个TCP流对应一个TCP连接）
...
右键点击某个HTTP数据包，选择跟踪TCP流查看HTTP会话的数据，分析浏览器与WEB服务器之间进行了<ModernInput size="small" />次HTTP会话（一对HTTP请求和响应对应一次HTTP会话）
