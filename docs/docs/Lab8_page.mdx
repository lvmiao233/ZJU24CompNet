...
理论上，RESTful是一种非常优雅和清晰的设计，但在复杂的现实网络环境中，开发者有时会做出一些“妥协”，由此便诞生了所谓“POSTful”的实践（没有POSTful这个术语），即无论什么操作，都使用POST方法一把梭，这通常是出于一些无奈的原因，比如一些运营商或企业安全设备可能会对GET/POST以外的方法进行误处理，或是中间代理服务器对GET请求的异常缓存行为都可能影响业务的正常运行；在这种情况下，`GET /users` 可能会被改造为 `POST /getUserList`，`DELETE /users/123` 会被改造为 `POST /deleteUser` 等等
