...
description: 计算机网络实验八（Lab8）指导手册，内容为实现一个轻量级的Web服务器。本实验旨在通过从零开始构建Web服务器，深入理解HTTP协议和Web服务架构。内容涵盖HTTP请求解析、响应构建、静态资源处理、并发管理以及对不同HTTP方法和特性的支持。
---
...
import CollapseList from "@site/src/components/CollapseList";
import ApiTestRunner from "@site/src/components/ApiTestRunner";
...
<DeadlineProgress start={'2025-09-30 18:50:00'} end={'2025-12-30 23:59:59'}/>
...
<GridContainer>
    ...
    <FileCard file_type={'pdf'} name={'Lab8 实现一个轻量级的Web服务器 实验课件'} size={'2584436'} link={require('@site/assets/slides/Lab8-WebServer.pdf').default} />
...
<ApiTestRunner header={'快速完整测试'} testUri={'/test/lab8/resource-retrieve'} innerStyle={{marginTop: -20, marginBottom: -20}}/>

:::

<details>
<summary>实验理论讲解视频</summary>

<video
  width="100%"
  controls
  preload="metadata" 
  poster={require('./img/Lab8/cover.png').default}>
  <source src="https://oss.zjucomp.net/Lab8_WebServer.mp4" type="video/mp4" />
  抱歉，您的浏览器不支持嵌入式视频。
</video>
...
<CollapseList items={HTTPVersions} defaultActiveKey={['HTTP/1.0']} size={'small'} />
...
<img src={require("./img/Lab8/HTTP_persistent_connection.png").default} alt="HTTP 持久连接示意图" style={{zoom: "24%"}} />
...
<img alt={"Chrome 开发者工具中查看网页使用的 HTTP 版本的示例截图"} src={require('./img/Lab8/image-20241028145513246.png').default} style={{zoom: '60%'}}/>
...
<img alt={"HTTP 报文结构示意图（请求/响应起始行、头部与实体）"} src={require('./img/Lab8/image-20241029212218140.png').default} style={{zoom: '70%'}}/>
...
<ApiTestRunner header={"测试2 HTTP请求结构解析"} testUri={"/test/lab8/structure-parse"}/>
...
<CollapseList items={HTTPMethods} defaultActiveKey={['GET方法']} extras={HTTPMethodsExtras} size={'small'}/>
...
<details>
<summary>RESTful是什么？</summary>

你可能已经注意到了——POST方法能携带数据并返回响应，看起来我们提到的其他方法似乎都可以通过POST方法加上不同的参数来实现，那么，为什么要专门设计这么多不同的HTTP方法呢

这背后最主要的原因，就是一种被称为**表述性状态传递**（Representational State Transfer, REST）的软件架构风格，在RESTful架构中，网络上的所有事物都被视为**资源**，而每一个URI都代表一种特定的资源，我们对资源的操作，则通过不同的HTTP方法来定义，这使得接口变得直观且标准化

例如，对于一个/users资源

```yaml
GET    /users          # 获取所有用户列表 (查询)
POST   /users          # 新建一个用户 (新增)
GET    /users/123      # 获取ID为123的用户信息 (查询特定)
PUT    /users/123      # 完整替换ID为123的用户信息 (完整更新)
PATCH  /users/123      # 部分修改ID为123的用户信息 (部分更新)
DELETE /users/123      # 删除ID为123的用户 (删除)
```
RESTful的关键特性还包括：客户-服务器模式，无状态通信，缓存机制，统一接口，分层系统，[可选]按需代码

理论上，RESTful是一种非常优雅和清晰的的设计，但在复杂的现实网络环境中，开发者有时会做出一些“妥协”，由此便诞生了所谓“POSTful”的实践（没有POSTful这个术语），即无论什么操作，都使用POST方法一把梭，这通常是出于一些无奈的原因，比如一些运营商或企业安全设备可能会对GET/POST以外的方法进行误处理，或是中间代理服务器对GET请求的异常缓存行为都可能影响业务的正常运行；在这种情况下，`GET /users` 可能会被改造为 `POST /getUserList`，`DELETE /users/123` 会被改造为 `POST /deleteUser` 等等

现实世界没有完美的解决方案，根据实际情况做最适合的选择才是工程实践的常态，不过在我们的实验中，你只需要正确区分GET和POST方法，并执行相应的功能逻辑即可
...
* **资源属性**  如资源的类型、编码方式、最后修改时间等，帮助接收方正确处理接收到的数据
    ...
    <CollapseList items={resourceAttrInfo} size={'small'} extras={resourceAttrExtra}/>

* **缓存属性**  指示资源是否能/如何被缓存，从而提高网络性能，优化用户体验

    <CollapseList items={cacheAttrInfo} size={'small'} extras={cacheAttrExtra}/>

* **安全/隐私属性**  如要求身份验证、限制跨域请求等，用于增强用户的安全性和隐私保护

    <CollapseList items={securityAttrInfo} size={'small'} extras={securityAttrExtra}/>

* **机制协商**  如接收方的内容类型偏好或处理能力，以便发出方选择最适合的内容进行响应

    <CollapseList items={negotiationAttrInfo} size={'small'} extras={negotiationAttrExtra}/>
...
为了检验你对头字段的解析，我们不妨采用网络编程中一种被称为“回声协议(Echo Protocol)”或“镜像调试(Mirror Debugging)”的经典技巧；在开发复杂的通信协议时，让服务器原样回显它所接收到的数据是验证解析器是否正确、无误地听懂了客户端请求的最直接方式，虽然这不是一个真正的Web服务器功能，但能帮助我们确信协议解析这一基础环节是稳固可靠的；实现这个简单的WebEcho功能的具体步骤如下：
...
<ApiTestRunner header={"测试3 服务器的WebEcho功能"} testUri={"/test/lab8/web-echo"}/>
...
<img src={require('@site/docs/docs/img/Lab8/image-20241104011024024.png').default} alt="nmap 扫描开放端口的示例输出（试探扫描）" style={{zoom: "70%"}} />
...
为了满足自动化测试的需求，请你按照以下表格中的映射关系，对相应资源URL进行映射，我们测试的请求将使用映射后的URL（你完全可以选择其他映射方式，但需要自己进行测试）；这里的文件组织方式模仿了小型项目的常见组织方式——开发者为了内部管理方便常会按类型（如html, img）组织文件，但对用户呈现的URL则会设计得更简洁、友好且不暴露内部结构
...
<LinkCard title={'HTTP response status codes'} url={'https://developer.mozilla.org/en-US/docs/Web/HTTP/Status'}>{"HTTP response status codes indicate whether a specific HTTP request has been successfully completed."}</LinkCard>

实验中，你需要对请求URL进行验证，如果解析得到的是`GET`方法，则验证相应文件是否存在；如果解析得到的是`POST`方法，则验证接口URI是否为`/dopost`，如不匹配，则返回`404 Not Found`，否则均返回`200 OK`

<ApiTestRunner header={"测试4 资源URI映射"} testUri={"/test/lab8/uri-mapping"}/>

为了便于你分部分调试，我们不妨分开来独立测试路径映射、文件读取与内容传输功能，这里请你先实现资源URI映射；我们的测试框架将会对给定URL检验映射的正确性，具体来说，对于`GET`方法的请求，请你检验URL合法性，根据请求URL映射得到相应的文件路径，并将相应的文件路径的**字符串**（不是文件本身）返回给客户端，请求头需正确设置`Content-Type`字段（字符串长度）和`Content-Type`字段（`text/plain`）的值
...
* **构建HTTP响应**：根据读取到的文件内容构建HTTP响应报：文件字节流完整置于响应正文中；响应头`Content-Length`设置为文件大小（以字节为单位）；根据文件类型，设置响应头`Content-Type`，以指示客户端如何处理该资源，对于本实验，你无需实现复杂的MIME类型检测。只需根据请求文件的扩展名进行简单的映射即可，例如文本文件应设置`Content-Type: text/plain`，图片文件应设置`Content-Type: image/jpeg`等，请至少支持实验所需文件（.html, .txt, .jpg）的类型映射

<ApiTestRunner header={"测试5 资源访问"} testUri={"/test/lab8/resource-retrieve"}/>
...
打开多个浏览器窗口（命令：`1..4 | ForEach-Object { Start-Process msedge -ArgumentList "--new-window http://Web服务器IP:端口/index.html" }`可使用Edge批量打开4个窗口加载目标页面），同时访问包含图片的HTML文件（`Web服务器IP:端口/index.html`），并截图浏览器显示内容、TCP链接情况（命令同测试5，截取与监听端口相关的）
