...
但是，当Switch1要将这个帧通过另一个接入端口转发给Switch2时，它不会携带任何VLAN相关信息，因为接入端口默认连接的是终端设备（如PC），这些设备是无法识别VLAN标签的；所以，当这个“裸奔”的数据帧到达Switch2时，Switch2无法知道它在Switch1那边是属于VLAN 2的，只会根据自己端口的默认设置来处理，VLAN信息就这样“丢失”了——原始的VLAN归属信息留在了Switch1的内存里，没有跟着数据帧一起走；此时被误认为不属于VLAN 2的这个数据包自然无法到达PC4
