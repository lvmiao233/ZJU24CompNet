...
在PC机上运行Putty软件，选择Serial方式，默认为9600, COM1。按两下回车，检查是否已经连上交换机。并输入`enable`命令进入到特权模式。如果有密码，请参考[4.清除密码步骤](/docs/Lab2_page#part-1--单交换机)。

输入命令`show version`查看当前交换机型号信息并记录：

设备型号：<ModernInput size="large" questionId="lab2-device-model"/>，IOS软件版本：<ModernInput size="large" questionId="lab2-ios-version"/>，

软件映像文件名：<ModernInput size="large" questionId="lab2-image-file"/>，端口数量：<ModernInput size="large" questionId="lab2-port-count"/>。
...
<ScreenshotCard 
  questionId="Lab2-p1-s3"
  title="交换机文件系统截图"
  uploadOptions={[
    { id: 'flash', label: '上传文件系统截图' }
  ]}
>
  ![img](img/1.3.jpg)
</ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p1-s4"
  title="VLAN配置截图"
  uploadOptions={[
    { id: 'vlan', label: '上传VLAN配置截图' }
  ]}
>
  ![img](img/1.4.jpg)
</ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p1-s5"
  title="端口关闭测试截图"
  uploadOptions={[
    { id: 'interface-status', label: '上传端口状态截图' },
    { id: 'ping-result', label: '上传Ping测试结果截图' }
  ]}
/>
...
<ScreenshotCard 
  questionId="Lab2-p1-s6"
  title="端口启用测试截图"
  uploadOptions={[
    { id: 'interface-up', label: '上传端口启用状态截图' },
    { id: 'ping-success', label: '上传Ping成功结果截图' }
  ]}
></ScreenshotCard>
...
<ModernInput codeEditor={true} initialLines={2} questionId="lab2-task7-cmd1"/>

<ScreenshotCard 
  questionId="Lab2-p1-s7"
  title="交换机IP配置截图"
  uploadOptions={[
    { id: 'ip-config', label: '上传IP配置命令截图' }
  ]}
></ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p1-s8"
  title="远程登录配置截图"
  uploadOptions={[
    { id: 'telnet-config', label: '上传Telnet配置命令截图' }
  ]}
></ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p1-s9"
  title="Telnet连接成功截图"
  uploadOptions={[
    { id: 'telnet-success', label: '上传Telnet连接成功截图' }
  ]}
></ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p1-s10"
  title="Wireshark抓包截图"
  uploadOptions={[
    { id: 'wireshark-capture', label: '上传Wireshark抓包截图' }
  ]}
></ScreenshotCard>
...
<ModernInput codeEditor={true} initialLines={3} questionId="lab2-task11-cmd1"/>

<ScreenshotCard 
  questionId="Lab2-p1-s11"
  title="端口镜像配置截图"
  uploadOptions={[
    { id: 'mirror-config', label: '上传镜像配置命令截图' },
    { id: 'mirror-capture', label: '上传镜像后抓包截图' }
  ]}
></ScreenshotCard>

</TaskCard>

<TaskCard number={12} title="关闭端口镜像功能" needRecord={true}>

关闭PC1端口的镜像功能（命令：`no monitor session 1 destination interface 端口`），否则该端口不能正常收发数据。

请记录输入的命令：

命令1：<ModernInput codeEditor={true} questionId="lab2-task12-cmd1"/>

<ScreenshotCard 
  questionId="Lab2-p1-s12"
  title="关闭镜像功能截图"
  uploadOptions={[
    { id: 'disable-mirror', label: '上传关闭镜像命令截图' }
  ]}
></ScreenshotCard>
...
<ModernInput codeEditor={true} initialLines={5} questionId="lab2-task13-cmd"/>

<ScreenshotCard 
  questionId="Lab2-p1-s13"
  title="VLAN隔离测试截图"
  uploadOptions={[
    { id: 'vlan-config', label: '上传VLAN配置命令截图' },
    { id: 'ping-same-vlan', label: '上传同VLAN连通性截图' },
    { id: 'ping-diff-vlan', label: '上传不同VLAN隔离截图' }
  ]}
></ScreenshotCard>
...
<ModernInput codeEditor={true} initialLines={5} questionId="lab2-task14-config"/>
...
<ScreenshotCard 
  ...
  title="双交换机VLAN配置截图"
  uploadOptions={[
    { id: 'switch1-vlan', label: '上传Switch1的VLAN数据截图' },
    { id: 'switch2-vlan', label: '上传Switch2的VLAN数据截图' },
    { id: 'ping-cross-switch', label: '上传跨交换机连通性测试截图' }
  ]}
/>
...
<ModernInput codeEditor={true} initialLines={3} questionId="lab2-task16-cmd"/>

<ScreenshotCard 
  questionId="Lab2-p2-s16"
  title="VLAN Trunk配置截图"
  uploadOptions={[
    { id: 'trunk-config', label: '上传Trunk配置命令截图' },
    { id: 'ping-after-trunk', label: '上传Trunk后连通性测试截图' }
  ]}
>
</ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p2-s17"
  title="Switch1的STP状态截图"
  uploadOptions={[
    { id: 'switch1-stp', label: '上传Switch1的STP状态截图' }
  ]}
>
  ![img](img/2.17.1.jpg)
</ScreenshotCard>
...
<ScreenshotCard 
  ...
  uploadOptions={[
    { id: 'switch2-stp', label: '上传Switch2的STP状态截图' }
  ]}
>
  ![img](img/2.17.2.jpg)
</ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p2-s18"
  title="关闭STP影响测试截图"
  uploadOptions={[
    { id: 'disable-stp', label: '上传关闭STP命令截图' },
    { id: 'ping-delay', label: '上传Ping延迟测试截图' }
  ]}
>
</ScreenshotCard>
...
<ScreenshotCard 
  questionId="Lab2-p2-s19"
  title="STP恢复测试截图"
  uploadOptions={[
    { id: 'ping-recovery', label: '上传Ping恢复正常截图' }
  ]}
/>
...
<ScreenshotCard 
  questionId="Lab2-p2-s20"
  title="STP故障恢复截图"
  uploadOptions={[
    { id: 'switch1-recovery', label: '上传Switch1故障恢复状态截图' },
    { id: 'switch2-recovery', label: '上传Switch2故障恢复状态截图' }
  ]}
/>
...
<ModernInput codeEditor={true} initialLines={4} questionId="lab2-task21-switch1-cmd"/>

请记录Switch2输入的命令：

<ModernInput codeEditor={true} initialLines={4} questionId="lab2-task21-switch2-cmd"/>

<ScreenshotCard 
  questionId="Lab2-p2-s21"
  title="端口优先级配置截图"
  uploadOptions={[
    { id: 'switch1-priority', label: '上传Switch1优先级配置截图' },
    { id: 'switch2-priority', label: '上传Switch2优先级配置截图' }
  ]}
/>
...
<ScreenshotCard 
  questionId="Lab2-p2-s22"
  title="优先级配置效果验证截图"
  uploadOptions={[
    { id: 'switch1-priority-effect', label: '上传Switch1优先级效果截图' },
    { id: 'switch2-priority-effect', label: '上传Switch2优先级效果截图' }
  ]}
/>
...
<ScreenshotCard 
  questionId="Lab2-p2-s23"
  title="负载均衡切换测试截图"
  uploadOptions={[
    { id: 'switch1-failover', label: '上传Switch1故障切换状态截图' },
    { id: 'switch2-failover', label: '上传Switch2故障切换状态截图' }
  ]}
/>
...
<ModernInput codeEditor={true} initialLines={5} questionId="lab2-task24-switch1-config"/>

请记录Switch2的运行配置：

<ModernInput codeEditor={true} initialLines={5} questionId="lab2-task24-switch2-config"/>
