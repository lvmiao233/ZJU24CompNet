...
<DeadlineProgress start={'2025-10-14 18:50:00'} end={'2025-10-28 23:59:59'}/>
<GridContainer>
    ...
    <FileCard file_type={'pdf'} name={'Lab2 使用二层交换机组网 实验课件'} size={'1223654'} link={require('@site/assets/slides/luxq_Lab2.pdf').default} />
</GridContainer>
...
:::warning 仅两端设备均不支持自动MDIX时才需要使用交叉线，如果你使用了白色机身的CX3560系列设备，请直接选用直通线，尽量不要使用任何交叉线，以免交叉线老化影响实验正常进行
:::
...
由于实验室的网络设备为公用设备，在你进行实验前，可能已经有多组同学在设备上进行了配置，请你务必先使用`show running-config`查看当前实际配置，并适当撤销部分配置，以免干扰正常实验
...
5. 输入命令`show version`查看当前交换机型号信息并记录：
      * 设备型号：<AnswerInput size="large" questionId="lab2-s2-device-model"/>（进行资产管理和故障申报的基础信息）
      * IOS软件版本：<AnswerInput size="large" questionId="lab2-s2-ios-version"/>（用于判断设备是否存在已知的安全漏洞（CVE）或软件Bug）
      * 软件映像文件名：<AnswerInput size="large" questionId="lab2-s2-image-file"/>（在需要升级或恢复系统时，必须知道正确的文件名）
      * 端口数量：<AnswerInput size="large" questionId="lab2-s2-port-count"/>（进行网络规划的基础信息）
...
- **控制广播风暴**：这是VLAN最主要的功能，将网络划分为多个VLAN，广播流量就被限制在各自的VLAN内部，极大地减少了不必要的流量，提升了网络整体性能
- **增强网络安全**：不同VLAN之间的设备在二层上是完全隔离的，无法直接通信，这就像为不同部门或用户组建立了逻辑上的“墙”，防止了未授权的访问
- **提升网络管理的灵活性**：VLAN允许网络管理员根据部门、功能或项目等逻辑关系来组织网络，而不是受限于用户的物理位置，当一个员工从一楼搬到三楼时，无需重新布线，只需在交换机上修改端口的VLAN配置，就可以让他继续留在原来的工作网络中，极大地简化了网络管理
...
如果存在其它VLAN配置（请忽略序号1000+的VLAN，此部分VLAN为特殊用途预留，与本次实验无关且也无法删除，不会影响实验结果），请按如下步骤操作：

1. 进入非VLAN1端口的端口配置模式（命令：`interface <interface-id>`，如 `interface fa0/1`），将该接口还原为VLAN 1（命令：`switchport access vlan 1`）；如部分接口在VLAN数据库中不存在，也请你使用该方式将其还原访问VLAN1

2. 进入全局配置模式，使用 `no vlan <vlan-id>` 命令（例如 `no vlan 2`）将其删除，以保证实验环境的纯净
...
2. 为各台PC配置唯一的、在同一个子网内的静态IP地址，确保它们在同一个子网内（例如192.168.1.x，子网掩码均为255.255.255.0）
   ...
   <summary> 配置示例与提示 </summary>
    ...
    部分PC有多个有线网卡，请确保你配置的是网线实际连接到的网卡（即未处于网线已拔出状态）；如果提示“对相同子网配置了不同的默认网关”等问题，请配置另一个有线网卡，将其IP分配改为自动获取，清除其原有IP避免干扰实际使用网卡的静态IP配置

    配置完成后，请使用`ipconfig`命令确认配置已实际生效（如果保存时有报错或警告，此时你的IP配置通常完全不生效），避免实际使用了错误的IP影响实验的正确进行
...
3. 在PC上使用`ping`命令测试彼此之间的连通性，确保所有PC都能互相Ping通，如果Ping不通，请仔细检查IP配置和网线连接
    ...
    对于实验室的笔记本电脑，建议先**禁用其无线网卡**，以免同时连接有线网与校网导致非预期行为

    请检查确保涉及的PC**已关闭网络防火墙**（Windows安全中心-防火墙和网络保护-关闭全部类型网络的防火墙），否则无法响应ICMP请求
...
在未主动配置链路Cost的情况下，两条链路的Cost应当保持一致，如果出现不一致，你使用的网线可能存在故障（即8芯线中有1-4芯线短路或断路），导致网线被降级为低速率等级使用，自动匹配了较低的Cost值，需要更换网线以正常完成实验
...
* 端口状态显示为administratively down，意味着什么意思？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q1"/>
* 在交换机配置为镜像端口前，为什么可以抓取到其他PC之间的ARP请求包，而不能抓取ARP响应包？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q2"/>
* PC属于哪个VLAN，是由PC自己可以配置的，还是由交换机决定的？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q3"/>
* 同一个VLAN的PC，如果配置了不同长度的子网掩码，能够互相Ping通吗？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q4"/>
* 为什么在划分为2个VLAN后，两组PC之间就不能进行IP通信了呢？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q5"/>
* 交换机在VLAN Trunk模式下使用的封装协议是什么？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q6"/>
* 未启用STP（Spanning Tree Protocol）协议时，交换机之间连接了多条网线后，为什么Ping测试的响应会延迟很大甚至超时？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q7"/>
* 从插上网线后开始，交换机的端口状态出现了哪些变化？大约需要多少时间才能成为FWD状态？期间，连接在该端口的计算机是否能够Ping通？
  <AnswerInput size="exlarge" questionId="lab2-analysis-q8"/>

## 4 讨论与心得

在完成本实验后，你可能会有很多待解答的问题，你可以把它们记在这里，接下来的学习中，你也许会逐渐得到答案的，同时也可以让老师了解到你有哪些困惑，老师在课堂可以安排针对性地解惑。等到课程结束后，你再回头看看这些问题时你或许会有不同的见解：

<AnswerInput size="exlarge" questionId="lab2-question"/>

在实验过程中你可能会遇到的困难，并得到了宝贵的经验教训，请把它们记录下来，提供给其他人参考吧：

<AnswerInput size="exlarge" questionId="lab2-experience"/>

你对本实验安排有哪些更好的建议呢？欢迎献计献策：

<AnswerInput size="exlarge" questionId="lab2-suggestion"/>

## 5 常见问题

* **实验过程中 STP 的收敛时间比较久，这是为什么，大型网络中不会影响日常使用吗？有没有比 STP 更快的协议？**


  STP 收敛慢主要是由其被动的计时器机制决定的，它依靠计时器（如 Max Age 20秒，Forward Delay 15秒）来确认链路状态，端口从侦听状态变为转发状态至少需要经历两个 Forward Delay 时长，即30秒到50秒，这是一种为了防止环路而采取的极其保守的等待策略；在大型网络中，几十秒的中断对于对延迟敏感的业务（如语音、视频会议、金融交易）是致命的，会导致连接超时或业务中断；为了解决 STP 收敛速度慢的问题，主要有两种更高效的协议：

  * **RSTP**即快速生成树协议，它对 STP 进行了重大改进，通过减少端口状态（仅保留 Discarding、Learning 和 Forwarding）以及引入 P/A 机制（Proposal/Agreement），将收敛时间从秒级缩短到了毫秒级或秒级，是目前最常用的替代方案
  * **MSTP**即多生成树协议，它在 RSTP 的基础上增加了对 VLAN 的支持，可以将多个 VLAN 映射到一个实例中，不仅收敛快，还能实现流量的负载分担，避免单一链路拥塞


* **配置端口镜像时，某个目的端口可以作为别的目的端口的源端口吗？**

  当一个端口被配置为镜像的目的端口时，该端口会进入镜像其他端口的特殊状态，原本的正常交换转发功能会受到限制或完全失效；目的端口主要负责传出数据，而作为源端口需要被监控传入的正常业务流量，如果一个端口既是终点又是起点，会导致镜像逻辑混乱或死循环，因此此类配置一般是直接禁止的

* **交换机和路由器有什么区别，交换机实际使用场景有哪些？**

  * **工作层级与对象**：交换机主要工作在数据链路层（二层），依据 MAC 地址表进行转发，处理的是数据帧；路由器工作在网络层（三层），依据路由表进行转发，处理的是数据包
  * **广播域处理**：交换机默认不隔离广播，属于同一个广播域；路由器可以隔离广播，每一个接口就是一个独立的广播域
  * **实际使用场景**：**交换机**多用于局域网内部，负责将大量的终端设备（电脑、打印机、摄像头）接入网络，接口高密度，数据交换速度极快；**路由器**主要用于不同网络之间的互联，例如连接企业内网与互联网（WAN），负责路径选择、NAT 转换以及复杂的安全控制

* **实验中使用了 dot1q 封装协议，那么另一个选项 ISL 协议与 dot1q 有什么区别？**

  ISL 是Cisco私有的封装协议，仅Cisco设备支持；dot1q 是 IEEE 发布的国际标准协议，几乎所有厂商的设备都支持，具有极好的兼容性；现代网络中，802.1Q（即 dot1q）占据绝对统治地位，ISL（Cisco私有的封装协议）则逐渐被淘汰，二者主要的区别有：

  * **封装方式**：**ISL** 采用外部封装模式，在原始以太网帧的头部和尾部加上总共 30 字节的头和尾，不修改原始帧内容；**dot1q** 采用内部标记模式，它在原始以太网帧的源 MAC 地址和类型字段之间插入 4 字节的 Tag，开销更小
  * **Native VLAN 支持**：ISL 不支持 Native VLAN 的概念，所有帧都会被封装；dot1q 支持 Native VLAN，属于该 VLAN 的数据帧在传输时不打标签，这能提高部分特定流量的转发效率

## 6 导出实验报告

<ExportButton templatePath={require('@site/assets/base_report/Lab2.zip').default} labName="Lab2" labId="lab2-" allowTeammate={true} />