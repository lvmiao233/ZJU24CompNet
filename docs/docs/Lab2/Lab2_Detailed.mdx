...
由于实验室的网络设备及其扩展卡存在差异，可用的端口范围可能有一定不同，对于我们的实验而言，E/FE/Gi (Ethernet/FastEthernet/GigibitEthernet)均可使用，不需要使用和示例绝对相同的接口编号

<PortsSvg style={{zoom: "20%"}} />

如图所示，接口的编号一般会在每列接口的上方/左右侧，且会有箭头灯指示数字对应上方或下方接口；对于接口前缀，你可以`show interfaces`查看
...
![Console接线](img/ConsolePorts.png)
