...
<FileCard file_type={'doc'} name={'Lab3 实验报告模板 Word版本'} size={'247296'} link={'http://10.214.0.253/network/download/Assignments/FundOfNetworks/2018/%e5%ae%9e%e9%aa%8c%e6%8a%a5%e5%91%8a%e6%a8%a1%e7%89%88_%e5%ae%9e%e9%aa%8c3.doc'} />
...
3. 将二层交换机和路由器连接的端口配置成VLAN Trunk模式（请参考实验2 进行配置并截图）。

   截图：配置命令（示例为GNS3模拟的配置截图，使用实际设备请参考实验2截图）

   ![](img/Lab3/image4.png)

   截图：配置后的结果（示例为GNS3模拟的结果截图，使用实际设备请参考实验2截图）
